...
public class TestMyServlet2 {
		...
		try {
			...
			for (Element element : childElements) {
				...
				if("servlet-mapping".equals(element.getName())) {
					//10.分别获取servlet-mapping元素节点的servlet-name和url-pattern的值
					String servletName = element.element("servlet-name").getText();
	...
	public void testMyServlet() {
		try {
			//1.模拟在浏览器输入一个url
			String urll = "/myServlet2";
			//2.将urlPattern作为key来获取servletClass
			String className = data.get(urll);
			//3.通过servletClass获取字节码文件
			Class clazz = Class.forName(className);
			//4.通过字节码文件创建实例对象
			Object obj = clazz.newInstance();
			//5.通过字节码文件获取方法（两个参术： 第一个是方法名称： 第二个参数是方法的参数）
			Method method = clazz.getMethod("init", null);
			//6.调用invoke方法执行实例对象里面的方法（前面写的方法init)[第一个是调用方法的实例对象， 第二是方法的实参]
